...
/*   Updated: 2019/01/18 17:30:01 by wbraeckm         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static int		should_follow(t_paths path, t_room *check, t_room *from)
{
	if (from->reverse && from->reverse == check->index)
		return (path[*path] & REVERSE_FLAG ? 1 : 2);
	if (check->nb_conn < 2 || check->used != 0
	...
	t_room *room)
{
	...
	{
		if ((ret = should_follow(path, &lem->rooms[room->connections[i] - 1],
			room)))
...
t_paths			bfs(t_lem *lem)
{
	...
	found = add_new_paths(lem, paths, current, &lem->rooms[lem->start - 1]);
	free(current);
	while (*paths && !found)
	{
		current = (t_paths)*((void**)paths[0]->content);
		found = add_new_paths(lem, paths, current,
			&lem->rooms[(current[*current] & NO_FLAG) - 1]);
